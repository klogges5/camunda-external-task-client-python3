...
class ExternalTaskWorker:
    ...
    def subscribe(self, topic_names, action, process_variables=None, variables=None, local_variables=None):
        while True:
            self._fetch_and_execute_safe(topic_names, action, process_variables, variables, local_variables)

        self._log_with_context("Stopping worker")  # Fixme: This code seems to be unreachable?

    def _fetch_and_execute_safe(self, topic_names, action, process_variables=None, variables=None, local_variables=None):
        try:
            self.fetch_and_execute(topic_names, action, process_variables, variables, local_variables)
    ...
    def fetch_and_execute(self, topic_names, action, process_variables=None, variables=None, local_variables=None):
        ...
        resp_json = self._fetch_and_lock(topic_names, process_variables, variables, local_variables)
    ...
    def _fetch_and_lock(self, topic_names, process_variables=None, variables=None, local_variables=None):
        self._log_with_context(f"Fetching and Locking external tasks for Topics: {topic_names} "
                               f"with Process variables: {process_variables}")
        return self.client.fetch_and_lock(topic_names, process_variables, variables, local_variables)
